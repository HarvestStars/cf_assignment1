...
plt.legend()
plt.title('Temperature vs Fitted Seasonal Model', fontsize=16)
plt.grid(True)
plt.savefig('figs/deterministic_temp_model.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()

# plot the residuals (T_hat)
plt.figure(figsize=(12, 4))
plt.plot(daily_df.index, daily_df['residuals'], label='$\hat{T}_t$', color='orange')
plt.xlabel('Date', fontsize=14)
plt.ylabel('Value', fontsize=14)
plt.title('Detrended Temperature Series $\hat{T}_t$', fontsize=16)
plt.legend()
plt.grid(True)
...
plt.title('$\hat{T}_t$ vs Fitted AR(1)', fontsize=16)
plt.legend()
...
plt.title('AR(1) Residuals', fontsize=16)
plt.xlabel('Date', fontsize=14)
plt.ylabel('Residuals', fontsize=14)
plt.legend()
plt.grid(True)
